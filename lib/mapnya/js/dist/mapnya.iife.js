    ...
    function Axis(parent, scales, _options) {
        ...
        var fill_scale = function () {
            ...
            if (options.extra.fill_by != null) {
                ...
                return d3.scale.ordinal().domain(country_id).range(_.map(country_id, function (id, i) {
                    var pos = id_column.indexOf(id);
                    if (pos != -1) return scale(fill_by_column[pos]);else return options.extra.no_data_color;
                }));
            } else {
                return d3.scale.ordinal().domain(country_id).range(options.extra.color);
            }
        }();
        ...
        model.append("clipPath").attr("id", clip_id).append("rect").attr({
            "x": 0,
            "y": 0,
            "width": options.width,
        ...
        model.attr("clip-path", "url(#" + clip_id + ")").append("g").attr("class", "map").style("z-index", options.z_index);

        var projection = d3.geo.mercator().center(options.extra.center).scale(options.extra.scale);
        ...
        model.select(".map").append("path").datum(graticule).attr({
            "d": path,
            "stroke": "#fff",
            "stroke-width": 2
        });

        var map = model.select(".map").selectAll("path").data(options.extra.map_data.features).enter().append('path').attr("d", path).attr("stroke", options.extra.stroke_color).attr("fill", function (d) {
            if (typeof d.properties.iso_a3 == "undefined") return options.extra.no_data_color;else return fill_scale(d.properties.iso_a3);
        });

        if (options.zoom) {
            parent.call(d3.behavior.zoom().scaleExtent(options.zoom_range).on("zoom", function () {
...
//# sourceMappingURL=mapnya.iife.js.map